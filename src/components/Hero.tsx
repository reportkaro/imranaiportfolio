...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Draw waves
      const drawWave = (amplitude: number, wavelength: number, speed: number, lineWidth: number, color: string, offsetY: number) => {
        ctx.beginPath();
        ctx.lineWidth = lineWidth;
        ctx.strokeStyle = color;
        
        for (let x = 0; x < width + 10; x += 3) { // Smaller increment for smoother waves
